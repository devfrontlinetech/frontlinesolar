...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const calloutEls = calloutRefs.current.filter(
        Boolean,
      ) as HTMLDivElement[];
      ...
      function drawScene(progress: number) {
        ...
        skyGrad.addColorStop(
          0,
          `rgb(${8 + warmth * 15}, ${12 + warmth * 8}, ${28 + warmth * 5})`,
        );
        skyGrad.addColorStop(
          0.5,
          `rgb(${15 + warmth * 35}, ${20 + warmth * 18}, ${45 + warmth * 10})`,
        );
        skyGrad.addColorStop(
          1,
          `rgb(${25 + warmth * 60}, ${30 + warmth * 30}, ${55 + warmth * 8})`,
        );
        ...
        if (starOpacity > 0.01) {
          stars.forEach((star) => {
            const twinkle =
              0.6 +
              0.4 * Math.sin(time * star.twinkleSpeed + star.twinkleOffset);
        ...
        // === SUN GLOW (large halo) ===
        const haloGrad = canvasCtx.createRadialGradient(
          sunX,
          sunY,
          0,
          sunX,
        ...
        haloGrad.addColorStop(
          0,
          `rgba(255, 220, 100, ${0.25 * dayness + 0.08})`,
        );
        haloGrad.addColorStop(
          0.25,
          `rgba(255, 180, 50, ${0.12 * dayness + 0.03})`,
        );
        ...
        for (let r = 0; r < rayCount; r++) {
          ...
          const grad = canvasCtx.createLinearGradient(
            Math.cos(angle) * sunR,
            Math.sin(angle) * sunR,
            Math.cos(angle) * (sunR + rayLen),
            Math.sin(angle) * (sunR + rayLen),
          );
        ...
        // === SUN BODY ===
        const sunGrad = canvasCtx.createRadialGradient(
          sunX - sunR * 0.2,
          sunY - sunR * 0.2,
          0,
          sunX,
        ...
        sunGrad.addColorStop(
          0.5,
          `rgba(255, 210, 90, ${0.9 * dayness + 0.25})`,
        );
        ...
        groundGrad.addColorStop(
          0,
          `rgb(${18 + warmth * 15}, ${22 + warmth * 8}, ${38})`,
        );
        ...
        const reflectGrad = canvasCtx.createRadialGradient(
          sunX,
          groundY,
          0,
          sunX,
        ...
        for (let i = 0; i < numPanels; i++) {
          ...
          const perspectiveFactor =
            1 - Math.abs(i - (numPanels - 1) / 2) * 0.04;
          ...
          const facingFactor = Math.max(
            0,
            Math.sin(Math.abs(tiltAngle)) * 0.5 + 0.5,
          );

          // === LIGHT RAYS from sun to panel ===
          if (dayness > 0.03) {
            const rayAlpha = 0.08 * dayness;
            const rayGrad = canvasCtx.createLinearGradient(
              sunX,
              sunY,
              panelX,
              panelPivotY - pH / 2,
          ...
          const faceGrad = canvasCtx.createLinearGradient(
            -pW / 2,
            -pH / 2,
            pW / 2,
            pH / 2,
          ...
          for (let c = 0; c < cols; c++) {
            for (let r = 0; r < rows; r++) {
              ...
              const cellGlow = canvasCtx.createRadialGradient(
                cx,
                cy,
                0,
                cx,
          ...
          if (dayness > 0.05) {
            ...
            const glowGrad = canvasCtx.createLinearGradient(
              0,
              -pH / 2,
              0,
              pH / 2,
          ...
          if (dayness > 0.12) {
            ...
            for (let p = 0; p < particleCount; p++) {
              ...
              const perpAngle = Math.atan2(
                targetBaseX - sunEdgeX,
                sunEdgeY - targetBaseY,
              );
              const wobble =
                Math.sin(pProgress * Math.PI * 2 + i + time) *
                8 *
                (1 - pProgress);
              ...
              // Particle glow
              const pGrad = canvasCtx.createRadialGradient(
                pX,
                pY,
                0,
                pX,
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center px-6 text-center">
        ...
        {/* Headline */}
        <h1 ref={headlineRef} className="text-display-2xl text-white max-w-5xl">
        ...
        >
          Premium N-Type TOPCon bifacial solar systems designed for Tamil Nadu's
          climate — proudly serving Karur and all of Tamil Nadu. Maximize your
          PM Surya Ghar subsidy with TANGEDCO net-metering experts.
        </p>
        ...
        <div ref={calloutsRef} className="absolute inset-0 pointer-events-none">
          <div
            ...
            className="absolute top-[20%] left-[10%] glass p-4 rounded-xl max-w-[200px] animate-float"
            style={{ opacity: 0 }}
          >
            ...
            <p className="text-white/60 text-xs">
              Industry-leading N-Type TOPCon cell technology
            </p>
          </div>
          <div
            ref={(el) => {
            ...
            className="absolute top-[30%] right-[12%] glass p-4 rounded-xl max-w-[200px] animate-float"
          ...
          >
            ...
            <p className="text-white/60 text-xs">
              Performance guaranteed with DCR-compliant modules
            </p>
          </div>
          <div
            ref={(el) => {
            ...
            className="absolute bottom-[25%] left-[15%] glass p-4 rounded-xl max-w-[200px] animate-float"
          ...
          >
            ...
            <p className="text-white/60 text-xs">
              Energy from both sides of the panel
            </p>
          </div>
          <div
            ref={(el) => {
            ...
            className="absolute bottom-[30%] right-[10%] glass p-4 rounded-xl max-w-[200px] animate-float"
          ...
          >
            ...
            <p className="text-white/60 text-xs">
              Tier-1 ALMM listed domestic manufacturing
            </p>
          </div>
        </div>
      </div>
